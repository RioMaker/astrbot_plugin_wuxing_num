...
- 冷灰背景、白色圆角卡片、轻量阴影与五行浅色底，采用现代简洁信息图布局。
...
- 根气卡片有同色描边和标签，流转图中央汇总根气、阴阳数量；右上角独立显示本次判定。
- 关系连线采用实线/虚线和文字双重编码，标签避开箭头；五类关系分别使用匹配的图例。
...
徽记图集保存在 `assets/icons/wuxing_emblems.png`，随插件发布；不依赖临时附件路径或外网。缺少或损坏时使用无水印的五行卡片，数字、属性和判定仍正常显示。来源、图集布局和生成提示词见 [素材说明](assets/icons/README.md)。
